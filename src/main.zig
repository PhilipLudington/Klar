...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, command, "run")) {
        ...
        try metaCommand(allocator, args);
...
const MetaQueryMode = union(enum) {
    tag: []const u8,
    module,
    related: []const u8,
    deprecated,
...
const DeclKind = enum {
    ...
    fn toString(self: DeclKind) []const u8 {
        return switch (self) {
            .function => "function",
            .test_decl => "test",
            .struct_decl => "struct",
...
fn metaCommand(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const stderr = getStdErr();
...
fn metaCollectRelated(
    ...
    // Collect all declarations that:
    // 1. Have the target name (the declaration itself)
    // 2. Have a `meta related(...)` that references the target
    for (module.declarations) |decl| {
        const info = getDeclNameAndMeta(decl);
...
fn metaOutputJsonMeta(out: std.fs.File, meta: []const ast.MetaAnnotation) !void {
    ...
    for (meta) |ann| {
        switch (ann) {
            ...
            .tag, .group_join, .group_def, .define => {},
        }
    }
