...
pub const FunctionDecl = struct {
    ...
    /// Whether this function has side effects (Kira `effect fn`).
    /// Pure functions (effect=false) are annotated with `meta pure` in generated code.
    effect: bool,
};
    ...
    if (root.object.get("functions")) |funcs_val| {
        if (funcs_val == .array) {
            for (funcs_val.array.items) |func_val| {
                ...
                // Parse effect flag (defaults to false = pure)
                const effect_val = func_obj.get("effect");
                const is_effect = if (effect_val) |ev| switch (ev) {
                    .bool => |b| b,
                    else => false,
                } else false;
...
pub fn generateKlarSource(allocator: Allocator, m: *const KiraManifest) ![]u8 {
    ...
    for (m.functions) |func| {
        // Emit purity comment
        if (func.effect) {
            try appendSlice(allocator, &output, "    // effect\n");
        } else {
            try appendSlice(allocator, &output, "    // pure\n");
        }
    ...
    for (m.functions) |func| {
        ...
        // Emit meta pure annotation for pure functions
        if (!func.effect) {
            try appendSlice(allocator, &output, "\nmeta pure");
        }
...
test "generateKlarSource with functions" {
    ...
    // Default (no effect field) → pure
    try std.testing.expect(std.mem.indexOf(u8, source, "    // pure\n    fn add(a: i32, b: i32) -> i32") != null);
...
test "generateKlarSource maps string type to CStr with wrapper" {
    ...
    // Raw extern uses __raw_ prefix with CStr types, with purity comment
    try std.testing.expect(std.mem.indexOf(u8, source, "    // pure\n    fn __raw_greet(name: CStr) -> CStr") != null);

    // Wrapper accepts/returns string, with meta pure annotation
    try std.testing.expect(std.mem.indexOf(u8, source, "meta pure\npub fn greet(name: string) -> string {") != null);
...
test "parseManifest parses effect flag" {
    const allocator = std.testing.allocator;
    const json =
        \\{
        \\  "module": "io",
        \\  "functions": [
        \\    {"name": "pure_add", "params": [{"name": "a", "type": "i32"}], "return_type": "i32", "effect": false},
        \\    {"name": "read_file", "params": [{"name": "path", "type": "string"}], "return_type": "string", "effect": true},
        \\    {"name": "no_flag", "params": [], "return_type": "void"}
        \\  ],
        \\  "types": []
        \\}
    ;
    ...
    // Explicit false
    try std.testing.expectEqualStrings("pure_add", m.functions[0].name);
    try std.testing.expect(!m.functions[0].effect);
    // Explicit true
    try std.testing.expectEqualStrings("read_file", m.functions[1].name);
    try std.testing.expect(m.functions[1].effect);
    // Missing field defaults to false (pure)
    try std.testing.expectEqualStrings("no_flag", m.functions[2].name);
    try std.testing.expect(!m.functions[2].effect);
}

test "generateKlarSource emits purity comments for mixed functions" {
    const allocator = std.testing.allocator;
    const json =
        \\{"module": "mixed", "functions": [
        \\  {"name": "add", "params": [{"name": "a", "type": "i32"}, {"name": "b", "type": "i32"}], "return_type": "i32", "effect": false},
        \\  {"name": "write_log", "params": [{"name": "msg", "type": "i32"}], "return_type": "void", "effect": true}
        \\], "types": []}
    ;
    const m = try parseManifest(allocator, json);
    defer m.deinit(allocator);
    ...
    // Pure function gets // pure comment
    try std.testing.expect(std.mem.indexOf(u8, source, "    // pure\n    fn add(") != null);
    // Effect function gets // effect comment
    try std.testing.expect(std.mem.indexOf(u8, source, "    // effect\n    fn write_log(") != null);
}

test "generateKlarSource emits meta pure on pure string wrapper" {
    const allocator = std.testing.allocator;
    const json =
        \\{"module": "slib", "functions": [
        \\  {"name": "upper", "params": [{"name": "s", "type": "string"}], "return_type": "string", "effect": false}
        \\], "types": []}
    ;
    const m = try parseManifest(allocator, json);
    defer m.deinit(allocator);
    ...
    // Wrapper gets meta pure annotation
    try std.testing.expect(std.mem.indexOf(u8, source, "meta pure\npub fn upper(") != null);
}

test "generateKlarSource omits meta pure on effect string wrapper" {
    const allocator = std.testing.allocator;
    const json =
        \\{"module": "fslib", "functions": [
        \\  {"name": "read_file", "params": [{"name": "path", "type": "string"}], "return_type": "string", "effect": true}
        \\], "types": []}
    ;
    const m = try parseManifest(allocator, json);
    ...
    // Effect wrapper does NOT get meta pure
    try std.testing.expect(std.mem.indexOf(u8, source, "meta pure") == null);
    // But does get // effect comment in extern block
    try std.testing.expect(std.mem.indexOf(u8, source, "    // effect\n    fn __raw_read_file(") != null);
    // Wrapper still generated
    try std.testing.expect(std.mem.indexOf(u8, source, "pub fn read_file(path: string) -> string {") != null);
}
