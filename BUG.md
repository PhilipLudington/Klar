...
## [x] Bug 5: Compiler memory leak during native codegen of optional/result types
...
The Klar compiler leaks memory during native compilation (`klar build`) when resolving optional (`?T`), `Result[T, E]`, `CPtr[T]`, or `COptPtr[T]` types. The leak originates in `resolveTypeExprDirect` within the LLVM codegen emitter. The compiled binary itself runs correctly -- this is a compiler-internal leak, not a runtime issue.

### Steps to Reproduce

1. Run `klar build examples/apps/optional_demo.kl -o build/optional_demo`
2. Observe the `error(gpa): memory address ... leaked` message in stderr

### Expected Behavior

Clean build with no memory leak diagnostics.

### Actual Behavior

Build succeeds but reports a leaked allocation:
```
...
The leak is in `resolveTypeExprDirect`, which heap-allocates type objects (`*Type`, `*ResultType`, `*CptrType`, `*CoptPtrType`) when resolving parameterized types. These allocations were never freed because `named_values` gets cleared between functions without destroying the `semantic_type` pointers.

### Fix

Added `resolved_type_allocs` tracking list to `Emitter` that records all heap allocations made by `resolveTypeExprDirect`. All tracked allocations are freed in `deinit()`.
